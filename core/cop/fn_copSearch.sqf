...
if(count _invs > 0) then
{
	...
	if(_illegal > 200000) then
	{
		[[getPlayerUID _civ,_civ getVariable["realname",name _civ],"35"],"life_fnc_wantedAdd",false,false] spawn life_fnc_MP;
	} else {
		[[getPlayerUID _civ,_civ getVariable["realname",name _civ],"34"],"life_fnc_wantedAdd",false,false] spawn life_fnc_MP;
	};
