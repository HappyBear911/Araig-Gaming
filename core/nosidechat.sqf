			...
			if (ctrlText ((findDisplay 63) displayCtrl 101) == "Side Channel") then 
			{
				...
				if (disconnect_me == 0) then {
					if (isNil "warn_one") then {
						warn_one = true;
						systemChat ("Do not speak in sidechat. This is the first and last warning!");
						[] spawn DS_slap_them;
				...
				if (disconnect_me >= 1) then {
					if (isNil "warn_last") then {
						...
						[] spawn DS_slap_them;
						//["beat04"] spawn DS_really_loud_sounds;
						["You wanted it so yes..."] spawn DS_double_cut;
						1 fademusic 10;
						1 fadesound 10;
						disableUserInput true;
						systemChat ("You have been warned. Think again for a minute was it worth it.");
						sleep 60;
