...
if(playerSide == west) then {
	...
	// Cops can repair without Toolkit
	...
	_refuelStations = nearestObjects[player, ["Land_fs_feed_F"], 5];
	if(_curTarget isKindOf "Car" && count _refuelStations > 0 ) then {
		_refuelPrice = round((1 - (fuel _curTarget)) * 2000);
		_Btn6 ctrlSetText format["Refuel ($%1)",_refuelPrice];
		_Btn6 buttonSetAction "[life_vInact_curTarget] spawn life_fnc_refuelVehicle; closeDialog 0;";
		...
		if(_curTarget isKindOf "Ship") then {
			...
			if(typeOf (_curTarget) in ["C_Kart_01_Blu_F","C_Kart_01_Red_F","C_Kart_01_Fuel_F","C_Kart_01_Vrana_F","B_Heli_Transport_03_F","B_Heli_Transport_03_unarmed_F","O_Heli_Transport_04_F","O_Heli_Transport_04_ammo_F","O_Heli_Transport_04_bench_F","O_Heli_Transport_04_box_F","O_Heli_Transport_04_covered_F","O_Heli_Transport_04_fuel_F","O_Heli_Transport_04_medevac_F","O_Heli_Transport_04_repair_F"]) then {
				...
				if(count crew _curTarget == 0 && !(canMove _curTarget)) then { _Btn6 ctrlEnable true;} else {_Btn6 ctrlEnable false;};
			};
		};
	};
	...
	if(_curTarget isKindOf "Ship") then {
 		...
 		if(typeOf (_curTarget) in ["C_Kart_01_Blu_F","C_Kart_01_Red_F","C_Kart_01_Fuel_F","C_Kart_01_Vrana_F","B_Heli_Transport_03_F","B_Heli_Transport_03_unarmed_F","O_Heli_Transport_04_F","O_Heli_Transport_04_ammo_F","O_Heli_Transport_04_bench_F","O_Heli_Transport_04_box_F","O_Heli_Transport_04_covered_F","O_Heli_Transport_04_fuel_F","O_Heli_Transport_04_medevac_F","O_Heli_Transport_04_repair_F"]) then {
 			...
 			if(count crew _curTarget == 0 && !(canMove _curTarget)) then {_Btn2 ctrlEnable true;} else {_Btn2 ctrlEnable false;};
 		};
 	};
	...
	if(_curTarget isKindOf "Car" && count _refuelStations > 0 ) then {
		_refuelPrice = round((1 - (fuel _curTarget)) * 2000);
		_Btn3 ctrlSetText format["Refuel ($%1)",_refuelPrice];
		_Btn3 buttonSetAction "[life_vInact_curTarget] spawn life_fnc_refuelVehicle; closeDialog 0;";
		if(vehicle player == player) then {
