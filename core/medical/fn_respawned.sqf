...
*/
private["_handle","_primWeap","_primItems"];
...
// Set drugged variable to FALSE if respawned
{
	player setVariable["isOnDrugs_"+_x,false,true];
...
switch(playerSide) do
{
	case west: {
		...
		// Only load the saved primary weapon if no medics online
		if({side _x == independent} count playableUnits == 0) then {
			...
			if (count life_gearOnDeath > 0) then {
				...
				if(_primWeap != "") then {_primHandle = [_primWeap,true,false,false,false] call life_fnc_handleItem;};
