...
switch(_type) do
{
	...
	case 2:
	{
		if(({side _x == west} count playableUnits) == 0) exitWith {hint format["The police is currently unavailable. Please try again later."];};
	...
	case 4:
	{
		if(({side _x == independent} count playableUnits) == 0) exitWith {hint format["Currently there is no doctor on duty. Please try it again later."];};
