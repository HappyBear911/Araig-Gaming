...
ctrlShow[2001,false];
if((lbCurSel 2022) == -1) exitWith {hint "You have not selected!";ctrlShow[2001,true];};
...
if(!life_use_atm) exitWith {hint "You've recently robbed the bank! You can not use an ATM.";ctrlShow[2001,true];};
if(!([_amount] call TON_fnc_isnumber)) exitWith {hint "You have not entered a valid number.";ctrlShow[2001,true];};
if(parseNumber(_amount) <= 0) exitWith {hint "You must enter the number of items.";ctrlShow[2001,true];};
