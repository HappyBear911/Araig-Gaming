...
_val = ctrlText 2010;
ctrlShow[2002,false];
if((lbCurSel 2023) == -1) exitWith {hint "You have not selected anyone!";ctrlShow[2002,true];};
_unit = lbData [2023,lbCurSel 2023];
_unit = call compile format["%1",_unit];
if((lbCurSel 2005) == -1) exitWith {hint "You have not selected any item.";ctrlShow[2002,true];};
...
if(!([_val] call TON_fnc_isnumber)) exitWith {hint "You have not entered a valid number.";ctrlShow[2002,true];};
if(parseNumber(_val) <= 0) exitWith {hint "You must enter the number of items.";ctrlShow[2002,true];};
...
hint format["You have given %1 %2 %3",_unit getVariable["realname",name _unit],_val,_type];
